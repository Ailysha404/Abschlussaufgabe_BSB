...
class Read(object):
    ...
    __slots__ = ["length", "gc", "name", "orig_quality", "quality", "sequenz", "mean_qual_trimmed", "mean_qual_untrimmed", "trimmed_length"]
    def __init__(self, content, phred, alphabet, orig_quality, trim_quality):
...
def qualitaet(content, alphabet, phred):
    ...
    """
    values = [alphabet[str(letter)] for letter in content]
    ...
    for kmer in scores:
        ...
        if counter == 3:
            ...
            if kmer_mean < trim_val:
                return list(scores)[0:item_index]
...
def calc_data(dict_reads, phred):
    ...
    for letter in bases:
        ...
        dict_base_pos[letter] = [np.nanmean(pack) for pack in mean]

    qual_pos = (qual_pos for qual_pos in list(zip_longest(*iter(mean_qual_trim_base),fillvalue=np.nan)))
    for item in qual_pos:
        qual_pos_sterr.append(np.nanstd(item))
        qual_pos_mean.append(np.nanmean(item))

    return name, orig_length, trim_length, orig_qual, trim_qual, gc_con, mean_qual_trim_base, sequences, qual_pos_sterr, qual_pos_mean, dict_base_pos

def Series(name, orig_length, trim_length, orig_qual, trim_qual, gc_con, mean_qual_trim_base, sequences, qual_pos_sterr, qual_pos_mean, dict_base_pos, phred):
    read_count = len(name)
...
def graph_basenvert(series_data):
    ...
    bases = ["A", "C", "G", "T", "N"]
    counter = 0
    #print(series_data["Max Length after Trimming"])
    x_values = [i for i in range(0,len(series_data[f"Mean A Content per Base after Trimming"])) if i % 5 == 0]

    #np.nanmean(pack) for pack in chunk_gen(mean, 5)
    fig = plt.figure()
    ax = fig.subplots(1,1)

    for letter in bases:
        y_values = (np.mean(series_data[f"Mean {letter} Content per Base after Trimming"][i:i+5]) for i in range(0, len(series_data[f"Mean A Content per Base after Trimming"]), 5))
        sns.lineplot(
...
def graph_nshare(series_data):
    fig = plt.figure()
    ax = fig.subplots(1,1)
    y_values = (np.mean(series_data[f"Mean N Content per Base after Trimming"][i:i+5]) for i in range(0, len(series_data[f"Mean A Content per Base after Trimming"]), 5))
    sns.lineplot(
            x=range(0,series_data["Max Length after Trimming"], 5),
            y=y_values,
            lw=0.7,
...
def main():
    ...
    file = open(arguments.Dateipfad, "r")
    phred = phred_bestimmung(file, ascii, phred)
    file.seek(0)
    content = ((lines.rstrip() for lines in file))
